...
export class ObjectStoreProvider {
    ...
    static initialize(credentials?: {
        ...
        if (!config.bucket) {
            console.warn('[ObjectStoreProvider] No bucket configured. Attachment upload/download will fail.');
            return;
        }
        ...
        console.log(`[ObjectStoreProvider] Initialized with bucket: ${this.bucket}`);
    }
    ...
    private static getCredentialsFromEnv(): {
        ...
        if (process.env.VCAP_SERVICES) {
            try {
                ...
                console.error('[ObjectStoreProvider] Failed to parse VCAP_SERVICES:', e);
            }
        }
    ...
    static async delete(contentId: string): Promise<void> {
        ...
        try {
            ...
            console.log(`[ObjectStoreProvider] Deleted: ${contentId}`);
        } catch (error) {
            console.error(`[ObjectStoreProvider] Failed to delete ${contentId}:`, error);
        }
    }
}
