...
export class AttachmentHandler {
    ...
    private async onGetUploadUrl(req: cds.Request) {
        ...
        try {
            ...
            console.log(`[AttachmentHandler] Generated upload URL for: ${contentId}`);

            return { contentId, url };
        } catch (error) {
            console.error('[AttachmentHandler] Failed to generate upload URL:', error);
    ...
    private async onGetDownloadUrl(req: cds.Request) {
        ...
        try {
            ...
            console.log(`[AttachmentHandler] Generated download URL for: ${attachment.fileName}`);

            return url;
        } catch (error) {
            console.error('[AttachmentHandler] Failed to generate download URL:', error);
    ...
    private async beforeDelete(req: cds.Request) {
        ...
        if (attachment?.contentId) {
            console.log(`[AttachmentHandler] Deleting from Object Store: ${attachment.contentId}`);
