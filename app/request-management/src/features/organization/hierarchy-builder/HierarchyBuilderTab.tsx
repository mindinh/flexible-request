...
import { ChevronLeft, ChevronRight, Save, Plus, FolderOpen, RefreshCw, ChevronUp } from 'lucide-react';
...
export function HierarchyBuilderTab() {
    ...
    const handleSave = useCallback(async () => {
        ...
        try {
            ...
            // Step 4: Create OrgHierarchy records from edges
    ...
    const handleLoadOrg = async (orgName: string) => {
        try {
            ...
            for (const h of hierarchies) {
                ...
                if (!nodeMap.has(pk)) nodeMap.set(pk, { entityType: h.parentType, entityId: h.parentType === 'GROUP' ? h.parentGroup_ID : h.parentUser_ID });
                if (!nodeMap.has(ck)) nodeMap.set(ck, { entityType: h.childType, entityId: h.childType === 'GROUP' ? h.childGroup_ID : h.childUser_ID });
            ...
            for (const [key, info] of nodeMap) {
                ...
                if (info.entityType === 'GROUP') {
                    ...
                    newNodes.push({
                        id: nid, type: 'groupNode', position: { x, y },
                        data: {
                    ...
                    newNodes.push({
                        id: nid, type: 'userNode', position: { x, y },
            ...
            const newEdges = edgesToCreate.map((e, i) => ({
                ...
                type: 'smoothstep', animated: true, style: { stroke: '#8b5cf6', strokeWidth: 2 },
                                        ...
                                        <div className="flex flex-col gap-0.5 mb-2">
                                            {store.savedOrgs.map((org) => (
                                                <button key={org.id}
                                                    ...
                                                    className={`flex items-center gap-2 w-full text-left px-2 py-1.5 rounded-lg text-[12px] transition ${store.currentOrgName === org.name
                                                            ? 'bg-blue-50 text-blue-700 font-medium'
                                                            : 'text-slate-600 hover:bg-slate-50'
                                                        }`}>
                                                    <FolderOpen size={12} className={store.currentOrgName === org.name ? 'text-blue-500' : 'text-slate-400'} />
